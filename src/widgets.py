...
class DEFAULT_TEXT_BUFFER(tkinter.Text):
	def __init__(self, parent, name, type="normal", render_parent=None):
		...
		self.mouse_drag_last = 0
		self.mouse_drag_last_reg = False
	...
	def font_size_set(self, arg=None):
		...
		self.see("insert")
		if (self.parent.buffer == self or self == self.parent.command_out or self == self.parent.command_entry):
	...
	def cursor_mode_set(self, arg=None):
		...
		if (self.parent.conf["cursor_style"] >= 4):
		...
		elif (self.parent.conf["cursor_style"] == 3): #TERMINAL-LIKE BLOCK HIGHLIGHT LINE
			# self.tag_configure("cursor", foreground=self.parent.theme["window"]["bg"])
			# self.tag_configure("cursor", background=self.parent.theme["window"]["fg"], foreground=self.parent.theme["window"]["bg"])
			self.tag_configure("cursor", background=self.parent.theme["window"]["insertbg"], foreground=self.parent.theme["window"]["bg"])
			self.block_cursor = True
	...
	def change_case(self, arg=None, case=None):
		...
		self.parent.unhighlight_chunk_main_thread(start_index=index_range[0], stop_index=index_range[1])
		self.lexer.lex()
	...
	def char_enclose_custom(self, arg=None, start="", end=""):
		self.sel_start = self.index("sel.first")
		index = self.index("sel.last")
		i = self.index("insert")

		self.insert(index, end)
		self.insert(self.sel_start, start)

		if (i != self.sel_start):
			self.mark_set("insert", f"insert -{len(end)}c") # sets the insert cursor back the length of the string we're enclosing with
	...
	def on_mouse_drag(self, arg=None, left=None, right=None):
		if (self.mouse_drag_last_reg): return
		self.mouse_drag_last_reg = True
		curr_dir = None
		x = arg.x
		# print(arg)
		time.sleep(0.2)
		self.mouse_drag_last = self.parent.winfo_pointerx() - self.parent.winfo_rootx()
		print("going: ", x, arg.x, self.mouse_drag_last)
			
		if (self.mouse_drag_last <= x):
			print("left")
			getattr(self, left)()
			
		elif (self.mouse_drag_last > x):
			print("right")
			getattr(self, right)()

		self.mouse_drag_last = arg.x
		self.after(500, lambda: setattr(self, "mouse_drag_last_reg", False))
...
class COMMAND_OUT(DEFAULT_TEXT_BUFFER):
	...
	def add_input(self, arg):
		...
		if (arg.keysym == "BackSpace"):
			...
			for ii in range(self.edits_since_last_input[-1]):
				try: self.edit_undo()
				except Exception: break

			self.edits_since_last_input.pop()
		...
		if (not self.input):
			self.highlight_tags()
			# for i in range(self.edits_since_last_input[-1]):
				# try: self.edit_undo()
				# except Exception: break
	...
	def remember_position(self, arg=None):
		self.last_index = self.index("insert")
		self.last_line = self.get("insert linestart", "insert lineend")

		return "break"

	def restore_position(self, arg=None):
		self.mark_set("match_end", "1.0")
		count = tkinter.IntVar()
		index = self.search(self.last_line, "match_end", "end", count=count, nocase=True)
		if index != "" and count.get() != 0: self.mark_set("insert", index)
		else: self.mark_set("insert", self.last_index)
	...
	def show_input(self):
		...
		# while (True):
			...
			# for tag in list(self.parent.theme["highlighter"]):
				# t = self.tag_nextrange(tag, index+" linestart", index+" lineend")
				# if (t):
					# tags.append([f"{match_count}.0", f"{match_count}.0 lineend", tag])
					# print(t, match_count)
		...
		while (True):
			...
			if (index == ""): break
			if (count.get()) == 0: break
			# print(self.index("match_end"), self.index(index))
			p.append([self.index("match_end+1c"), self.index(index+" linestart")])
			# print(p[-1])
			self.mark_set("match_end", index+" lineend")
		if (p): p.append([self.index("match_end linestart +1l"), "end"])

		self.edits_since_last_input.append(0)
		# print(p)
	...
	def add_stdout(self, arg=None, tags=None, justify="left"):
		...
		if (tags):
			...
			for i in range(len(tags)):
				...
				if (tags[i][2:]): self.tag_add(tags[i][2], tags[i][0], tags[i][1])
				else: self.tag_add("keywords", tags[i][0], tags[i][1])
...
class TEXT(DEFAULT_TEXT_BUFFER):
	...
	# def move(self, arg=None, mod=[], key=None):
		...
		if (prefix == ""): self.sel_start = None; del self.queue[:]
			...
			self.sel_start =self.index('sel.first')
			self.sel_end =self.index('sel.last')
		self.parent.update_index()
	...
	def undo(self, arg=None):
		...
		self.parent.unhighlight_chunk(start_index=start_index, stop_index=stop_index)
		self.lexer.lex()
	...
	def move_line(self, arg=None, up=True, line_offset=1):
		...
		if (self.tag_ranges("sel")):
			...
			self.parent.unhighlight_chunk_main_thread(start_index=self.convert_line_index("int", self.index("sel.first")), stop_index=self.convert_line_index("int", self.index("sel.last")))
			self.lexer.lex()
