...
class PARSER:
	def __init__(self, parent):
		...
		self.commands = { 
			...
			'(l(ist_)?)?(sub)?proc' : [self.list_subprocess, 'subproc'],
			'(create_t)?(e)?mp(_buffer)?' : [self.create_temp_buffer, 'creates temporary buffer | usage: create_temp | create_temp [buffer name]'],
	...
	def lex_print(self, arg=None):
		if (not arg[1:]): self.parent.buffer.lexer.print_res()
		elif (arg[1] in ["func", "fn", "funcs", "fns"]):
			self.parent.buffer.lexer.print_functions([arg[2:]])
		elif (arg[1] in ["obj", "objs"]):
			self.parent.buffer.lexer.print_objs([arg[2:]])
	...
	def lf(self, arg=None):
		self.parent.buffer.convert_to_lf()

	def crlf(self, arg=None):
		self.parent.buffer.convert_to_crlf()
